...
class OnPolicyDistillActor:
    ...
    def forward_backward_batch(
        ...
        if use_dynamic_bsz:
            ...
            if mpu.is_pipeline_last_stage():
                ...
                for i, mb in enumerate(micro_batches):
                    ...
                    calc_kl_mask = mb["attention_mask"].clone().to(bool)
                    calc_kl_mask[:, : (-response_length - 1)] = False
                    mb_["calc_kl_mask"] = calc_kl_mask
                    mb_["kl_losses"] = torch.zeros_like(calc_kl_mask, dtype=torch.float32)
                    mb_["teacher_topk_logps"] = teacher_topk_logps[i].pin_memory()
                    mb_["teacher_topk_indices"] = teacher_topk_indices[i].pin_memory()
                    micro_batches_.append(mb_)
                    # mb.set_("calc_kl_mask", calc_kl_mask)
                    # mb.set_("kl_losses", torch.zeros_like(calc_kl_mask, dtype=torch.float32))
                    # mb.set_("teacher_topk_logps", teacher_topk_logps[i].pin_memory())
                    # mb.set_("teacher_topk_indices", teacher_topk_indices[i].pin_memory())
        else:
            ...
            if mpu.is_pipeline_last_stage():
                ...
                for i, mb in enumerate(micro_batches):
                    ...
                    mb_["calc_kl_mask"] = calc_kl_mask
                    mb_["kl_losses"] = torch.zeros_like(calc_kl_mask, dtype=torch.float32)
                    mb_["teacher_topk_logps"] = torch.tensor(teacher_topk_logps[i]).pin_memory()
                    mb_["teacher_topk_indices"] = torch.tensor(teacher_topk_indices[i]).pin_memory()
                    micro_batches_.append(mb_)
                    # mb.set_("calc_kl_mask", calc_kl_mask)
                    # mb.set_("kl_losses", torch.zeros_like(calc_kl_mask, dtype=torch.float32))
                    # mb.set_("teacher_topk_logps", torch.tensor(teacher_topk_logps[i]).pin_memory())
                    # mb.set_("teacher_topk_indices", torch.tensor(teacher_topk_indices[i]).pin_memory())

        # compute input shapes for pp stages
        n_micro_batch = len(micro_batches_)
        ...
        # breakpoint()
        batch_generator = make_batch_generator(micro_batches_, vpp_size=len(self.actor_module))
